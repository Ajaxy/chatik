...
function setupActions (app) {
    var secret = app.get('secret'),
        actionGet = '/' + secret,
        actionPost = '/post/' + secret;

    app.get(actionGet, function (req, res) {
        return store.load()
            .then(function (strings) {
                res.render('index', {
                    content: prepare(strings, 'http://' + req.get('host')),
                    secret: secret
                });
    ...
    app.post(actionPost, function (req, res) {
        if (req.method == 'POST') {
...
function prepare (strings, baseUrl) {
    strings = _.map(strings, function (string) {
        ...
        string = string.replace(/^@(\w)([\w\-\._]+:)/gi, '<span class="login"><span class="first-letter">$1</span>$2</span>');
